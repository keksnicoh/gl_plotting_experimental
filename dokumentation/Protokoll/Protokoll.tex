...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=200px]{logi/xn28}
	\includegraphics[width=200px]{logi/xn32}
	\includegraphics[width=200px]{logi/xn345}
	\includegraphics[width=200px]{logi/xn38}
...
Betrachtet man den Graphen für $r=3.45$ stellt man fest, dass der Graph bei $r=3.05$ in diesem Graph wieder mehrfach wiederzufinden ist. $f^{\infty}$ scheint ein fraktales Objekt zu sein (Selbstänhnlichkeit). 
\begin{figure}[!htbp]
	\centering
...
Bildet die Funktion einen Punkt idempotent ab, so handelt es sich um einen Fixpunkt, es gilt: $f^n(x)=x$ $\forall n \iff x$ ist Fixpunkt. Des weiteren unterscheidet man zwischen stabilen und instabilen Fixpunkten. Ist der Fixpunkt stabil so konvergiert $f^n(x)$ für beliebige Anfangswerte bei steigendem $n$ gegen diesen Fixpunkt. Im Fall des instabilen Fixpunktes entfernt sich das Ergebnis nach jeder Iteration vom Fixpunkt.
\newline
Stabilitätsbedingung: Sei $x$ ein Fixpunkt, dann gilt
\begin{equation} |f'(x)|<1 \iff Fixpunkt-stabil \end{equation}
\begin{equation} |f'(x)|>1 \iff Fixpunkt-instabil \end{equation}
Dies lässt sich überprüfen, indem man $x$ als Störung des Fixpunktes ausdrückt und linear approximiert. Es sei $x_{n+1} = f(x_n)=f(x^*+\delta)$, wobei $x^*$ ein Fixpunkt von $f(x)$ ist. Dann folgt für die Taylorentwicklung 1ten Grades
\begin{equation}
...
\newline
Für die logistische Abbildung haben wir die Fixpunktgleichung$ f(x)=rx(1-x)=x $ folgen die Fixpunkte $x_{0}=0$ und $x_{1}=1-\frac{1}{r}$. Aus der Stabilitätsbedingung (wir betrachten $f'(x)=r-2rx$) folgt:
\begin{equation}
...
\caption{Analyse des Lyapunov Exponenten. Links: Konvergenz der analytischen Implementation Gleichung \ref{eq:lyapunov-analytisch} (Punkte) gegen Konvergenz der Definition Gleichung \ref{eq:lyapunov-def} (Linie, zur besseren visuellen Unterscheidung wurden die diskreten Abbildungspunkte mit Linien verbunden). Mitte: Kontrollparameter r aufgetragen gegen Definitionsbereich $x_0$. Je dunkler ein Punkt (r, $x_0$) desto besser konvergiert $\lambda(r,x_0)$ (nach Gleichung \ref{eq:lyapunov-renormiert}). Rechts: Analog zur Mittleren Abbildung für die analytische Implementation (sourcecode: prak/lyapunov-defintion-convergence.py, prak/lyapunov-rx-convergence.py)}.
\label{fig:lyapunov}
\end{figure}
...
\subsubsection{Vergleich der analytischen und renormierten Implementation}
...
\begin{equation}f(x_n)=x_{n+1}=r\sin(x)\end{equation}
Wir haben die bereits implementierten Programme nun auf die Sinus Funktion angewendet. Abbildung \ref{fig:bifurc-sin} zeigt das Bifurkationsdiagramm der Sinusabbildung. Es fällt sofort auf, dass sich die Bildpunkte innerhalb einer Einhüllenden befinden. Betrachtet man die Funktion wird wegen des Bildbereiches des Sinus ($[-1,1]$) sofort klar, dass $\sup_{n \in \mathbb{N}} |f^n(r)| \leq r $ gilt. Bei den Bifurkationsdiagrammen der logistischen Abbildung hatten wir ein festen Startwert $x_0$ gewählt. Für die Sinusabbildung wählten wir als Startwerte die Folge $x_{0,n} = (-1)^nx_0$, da sonst im Bereich $r \in [0,\pi]$ das Bild des Graphen stets positiv wäre. Auch dieses Phänomen lässt sich mit einfachen Überlegungen erklären: 
\begin{equation}\sin([0,\pi]) = [0,1] \Rightarrow f([0,\pi]) = [0,\pi] \Rightarrow f^2([0,\pi]) = [0,\pi] \forall r \in [0,\pi]\end{equation}
...
\begin{figure}[!htbp]
\includegraphics[scale=0.20]{duffing-awp2-500k-nach-500k-h0,01-runge}
\includegraphics[scale=0.20]{duffing-awp3-500k-nach-500k-h0,01-runge}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[scale=0.38]{duffing-phase-RK-lambda0,04-h0,001}
	\includegraphics[scale=0.38]{duffing-poincare-2000k-after100k-euler-lambda0,04-eps7,5}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[scale=0.50]{schaltskizze}
...
\begin{figure}[!htbp]
\centering
\includegraphics[scale=0.26]{schwing-runge-nach300k-weitere20k-10-9-1V}
\includegraphics[scale=0.26]{schwing-runge-nach300k-weitere20k-10-9-1,2V}
...
\begin{figure}[!htbp]
\centering
\includegraphics[scale=0.26]{schwing-runge-nach300k-weitere20k-10-9-20V}
\includegraphics[scale=0.26]{schwing-runge-nach300k-weitere20k-10-9-30V}
