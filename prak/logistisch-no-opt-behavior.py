...
"""
LOGISTISCH_MAP_SQUARED = """
uniform float r;
float g(float x) { return r*x*(1-x); }
vec4 f(vec4 x) {
    return vec4(x.x, g(g(x.x)), 0, 0.5);
}
"""
GERADE = """
vec4 f(vec4 x) {
...
window.plotter.add_graph('bifurkation2', graph.Line2d(cdomain, LOGISTISCH_MAP_SQUARED))
window.plotter.get_graph('bifurkation2').set_colors(color_min=[.0,0.0,.0,1], color_max=[0.0,.0,.0,1])
